...
testrunner.bat -s "LLCTDBill134 Test Suit" -c "UpdateDeal_ON" -PDealResponseFilePath="%DealResponseFilePath%" -PLenderId=%LenderId% -PLawyerID=%LawyerID% -PLenderReferenceNumber=%LenderReferenceNumber% -PMortgageNumber=%MortgageNumber% -PClosingDate=%ClosingDate% -PCity=%City% -PProvince=%Province% -PDocumentType=%DocumentType% -PFCTURN=%FCTURN% -PTotalLoanAmount=%TotalLoanAmount% -e%RequestURL% %projectFileName%